...
# -----------------------------
BASE_DATA_DIR = "/Users/mehdi/Desktop/Network_Analysis/TSD/data_new_v2"
CITY = "Winnipeg"  # <<< change ONLY this
...
BASE_OUT_DIR = "/Users/mehdi/Desktop/Network_Analysis/TSD/ML_result"
MODEL_CODE_DIR = "/Users/mehdi/Desktop/Network_Analysis/TSD/python_v2"

MAX_ITERS_PER_COMBO = 500
...
# AUTO (DO NOT EDIT)
# -----------------------------
UVT_FILE = os.path.join(BASE_DATA_DIR, CITY, INPUT_NAME)
OUT_DIR = os.path.join(
    BASE_OUT_DIR,
    f"online_results_{CITY}_joint_real_synthetic_interval_beam_repair"
)

if not os.path.exists(UVT_FILE):
    raise FileNotFoundError(f"UVT_FILE not found: {UVT_FILE}")
...
def build_node_time_features(
    ...
    for v in range(n):
        ...
        for t in range(1, T + 1):
            ...
            x[v, t, 7] = float(torch.log(torch.tensor((in_cnt + 1) / (out_cnt + 1), dtype=torch.float32)))
...
class BeamState:
    def __init__(
        self,
        intervals: Dict[int, Tuple[int, int]],
        accumulated_model_score: float = 0.0,
        steps: int = 0,
...
def generate_children(state: BeamState, scored_rows: List[Dict[str, Any]]) -> List[BeamState]:
    ...
    for row in enriched[:EXPAND_TOP_K]:
        ...
        child.steps += 1
        child.last_move = (row["v"], row["t"], row["newL"], row["newR"])

        sig = child.signature()
        if sig in seen_child_signatures:
            continue
        seen_child_signatures.add(sig)
...
def repair_shrink_intervals(
    ...
    vertices = sorted(current.keys(), key=lambda v: ((current[v][1] - current[v][0] + 1), v), reverse=True)
...
def write_result_txt(
    ...
    *,
    ...
    with open(out_path, "w", encoding="utf-8", newline="\n") as f:
        f.write("ML-GUIDED TEMPORAL SEPARATOR (ONLINE, JOINT REAL+SYNTHETIC TRAINING, INTERVAL-AWARE BEAM SEARCH + REPAIR V1)\n")
        f.write("=" * 80 + "\n")
        f.write(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"City: {city}\n")
        f.write(f"Input file: {input_file}\n")
        f.write(f"Combo: {combo['combo_id']}\n\n")

        f.write("INSTANCE\n")
        f.write(f"  source: {combo['source']}\n")
        f.write(f"  target: {combo['target']}\n")
        f.write(f"  deadline(travel_time): {combo['deadline']}\n\n")
...
def main():
    ...
    edges, gmeta, combos = read_uvt_temporal_graph_with_combos(UVT_FILE)
    ...
    print("=" * 80)
    print("ONLINE ML-GUIDED SOLVER (JOINT REAL+SYNTHETIC TRAINED MODEL)")
    print("=" * 80)
    print(f"City: {CITY}")
    print(f"Input: {UVT_FILE}")
    print(f"OUT_DIR: {OUT_DIR}")
    print(f"Combos: {len(combos)}")
    print(f"Model window_w: {model_w} | feat_dim: {feat_dim}")
    print("=" * 80)

    x_cache: Dict[Tuple[str, int, int], torch.Tensor] = {}
    ...
    for i, c in enumerate(combos, start=1):
        ...
        print("\n" + "-" * 80)
        print(f"[RUN {i}/{len(combos)}] COMBO {cid:02d} s={s0} z={z0} d(trt)={d}")
        print("-" * 80)

        t_total_0 = time.time()

        if s0 not in node2idx or z0 not in node2idx or s0 == z0 or d < 1:
            print("[SKIP] invalid combo")
            combo_rows.append({
                "combo": cid,
                "s": s0,
                "z": z0,
        ...
        cache_key = (CITY, s_idx, z_idx)
        if cache_key not in x_cache:
            x_cache[cache_key] = build_node_time_features(edges_idx, n, T, model_w, s_idx, z_idx)
        x_tensor = x_cache[cache_key].to(device)
        ...
        for step in range(1, MAX_ITERS_PER_COMBO + 1):
        ...
        if best_state.steps >= MAX_ITERS_PER_COMBO:
            status = "hit_iter_cap"

        out_txt = os.path.join(OUT_DIR, f"combo_{cid:02d}_s{s0}_z{z0}_d{d}.txt")
        write_result_txt(
            out_path=out_txt,
            city=CITY,
            input_file=UVT_FILE,
        ...
        print(
            f"[DONE] combo {cid:02d} | status={status} | "
        ...
        combo_rows.append({
            ...
            "d": d,
            "status": status,
            "verts_before": repair_stats["before_vertices"],
            "pairs_before": repair_stats["before_pairs"],
            "verts_after": repair_stats["after_vertices"],
            "pairs_after": repair_stats["after_pairs"],
            "iters": best_state.steps,
            "time": total_wall,
            "ml_time": ml_wall,
            "repair_time": repair_wall,
        })

    summary_path = os.path.join(OUT_DIR, "summary.txt")
    with open(summary_path, "w", encoding="utf-8", newline="\n") as f:
        f.write("SUMMARY (Online ML-guided solver, joint real+synthetic training, interval-aware beam search + repair v1)\n")
        f.write("=" * 80 + "\n")
        f.write(f"City: {CITY}\n")
        f.write(f"Input: {UVT_FILE}\n")
        ...
        f.write("\nCOMBOS\n")
        f.write("combo\ts\tz\td\tstatus\tverts_before\tpairs_before\tverts_after\tpairs_after\tsuccess_iters\ttotal_time_s\tml_time_s\trepair_time_s\n")
        for r in combo_rows:
            f.write(
                f"{r['combo']}\t{r['s']}\t{r['z']}\t{r['d']}\t{r['status']}\t"
                f"{r['verts_before']}\t{r['pairs_before']}\t"
                f"{r['verts_after']}\t{r['pairs_after']}\t"
                f"{r['iters']}\t{r['time']:.6f}\t{r['ml_time']:.6f}\t{r['repair_time']:.6f}\n"
            )

    print("\n" + "=" * 80)
    print("[DONE] All combos processed.")
